...
void RenderPositionCustomization(){
    ...
    for(int i = 0; i < nbSizePositionDatatArray; i++){
    ...
    if(UI::Button("+ : Add a position")){
        nbSizePositionDatatArray++;
        allPositionData.InsertLast(PositionData(1, "\\$071"));
        OnSettingsChanged();
    }
    if(UI::Button("- : Remove a position")){
        if(nbSizePositionDatatArray > 1){
            nbSizePositionDatatArray--;
            allPositionData.RemoveAt(nbSizePositionDatatArray);
            OnSettingsChanged();
        }
    }
}